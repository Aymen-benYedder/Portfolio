...
// The deployed sitemap is generated by the Astro sitemap integration during
// `npm run build` (dist/sitemap-index.xml -> dist/sitemap-0.xml). The legacy
// root sitemap.xml (from scripts/generate-sitemap.mjs) was removed — it listed
// non-existent draft/injected/body HTML URLs and was never deployed.
...
async function loadUrls(inputPath) {
  if (!inputPath) {
    // Prefer the built sitemap shard; fall back to the index, then fail clearly.
    const candidates = [
    ...
    throw new Error(
      'No built sitemap found. Run `npm run build` first (generates dist/sitemap-0.xml), or pass a sitemap/CSV path as an argument.'
    );
  }
